...
class _CommandDashboardPageState extends State<CommandDashboardPage> {
  ...
  void _connect() {
    _sub = ApiClient.shared.traceStream().listen((event) {
      if (!mounted) return;
      final payload = (event['payload'] as Map?)?.cast<String, dynamic>() ?? const {};
      final envelope = (payload['envelope'] as Map?)?.cast<String, dynamic>() ?? const {};
      ...
      final hyp = (payload['hypothesis'] ?? envelope['hypothesis']) as String?;
      final ts = DateTime.now();

      final details = <String, String>{};
      for (final key in const ['zone', 'type', 'severity', 'incident_id', 'candidate_id']) {
    ...
    return Scaffold(
      ...
      appBar: UtilBar(
        ...
        actions: [
          ...
          UtilIconButton(icon: Icons.refresh, tooltip: 'Refresh', onPressed: () {
            _refreshIncidents();
            _refreshResources();
          }),
          const SizedBox(width: PulseSpace.x2),
          UtilIconButton(icon: Icons.logout, tooltip: 'Sign out', onPressed: () async {
            session.token = null;
        ...
        children: [
          Container(
            ...
            child: TabBar(
              ...
              labelStyle: PulseTheme.label(color: PulseColors.pearl).copyWith(fontSize: 11),
  ...
  Widget _leftPane() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Text('Islamabad operations', style: PulseTheme.display(size: 22)),
            const Spacer(),
            Container(
              width: 6, height: 6,
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: PulseColors.lime.withValues(alpha: 0.4), blurRadius: 4)],
              ),
            ),
            const SizedBox(width: PulseSpace.x2),
            Text('LIVE', style: PulseTheme.label(color: PulseColors.lime)),
          ]),
          const SizedBox(height: PulseSpace.x1),
          EmDashLeader('${_incidents.length} active · ${_msTimestamp(_lastUpdate).split('.').first}'),
          ...
          if (_incidents.isEmpty)
            const Padding(
              padding: EdgeInsets.symmetric(vertical: PulseSpace.x6),
              child: Center(child: EmDashLeader('No incidents — run a scenario from above.')),
            )
          else
            for (final inc in _incidents)
              IncidentCard(
                ...
                onTap: () => setState(() => _selectedIncidentId = inc.id),
                dense: true,
              ),
          const SizedBox(height: PulseSpace.x5),
  ...
  Widget _tracePane() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Container(
            padding: const EdgeInsets.fromLTRB(PulseSpace.x4, PulseSpace.x4, PulseSpace.x4, PulseSpace.x3),
            decoration: const BoxDecoration(
              border: Border(bottom: BorderSide(color: PulseColors.hairline)),
            ),
            child: Row(
              children: [
                Container(
                  ...
                  child: Text('LIVE', style: PulseTheme.label(color: PulseColors.signal)),
                ),
                const SizedBox(width: PulseSpace.x3),
                Text('Antigravity trace', style: PulseTheme.display(size: 18)),
                const Spacer(),
...
class _ResourceGauges extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final entries = _displayOrder
        ...
        .toList();
    final maxV = entries.fold<int>(0, (m, e) => e.value > m ? e.value : m).clamp(1, 100);
    return Container(
      padding: const EdgeInsets.all(PulseSpace.x3),
      decoration: BoxDecoration(
        border: Border.all(color: PulseColors.hairline),
      ...
      child: Column(
        ...
        children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  SizedBox(
                    width: 110,
                    child: Text(e.key.replaceAll('_', ' '), style: PulseTheme.label()),
                  ),
                  SizedBox(
                    width: 36,
                    child: Text(
          ...
          if (entries.isEmpty) Text('— no roster loaded —', style: PulseTheme.dataXs()),
        ],
      ),
    );
  }
    ...
    return PopupMenuButton<String>(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Text('▸', style: PulseTheme.data(size: 13, color: PulseColors.signal, weight: FontWeight.w700)),
            const SizedBox(width: PulseSpace.x2),
            Text('RUN', style: PulseTheme.label(color: PulseColors.signal)),
            const SizedBox(width: PulseSpace.x1),
            Text('▾', style: PulseTheme.data(size: 11, color: PulseColors.signal)),
          ],
        ),
      ),
    );
